...
if bash -n /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh 2>&1; then
...
for func in "${REQUIRED_FUNCTIONS[@]}"; do
    if grep -q "^${func}()" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh; then
...
for call in "${MAIN_CALLS[@]}"; do
    if grep -A 50 "^main()" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | grep -q "${call}"; then
...
CHINESE_COUNT=$(grep -c "宝塔\|面板\|安装\|配置\|成功" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh || echo "0")
...
for pattern in "${LOG_PATTERNS[@]}"; do
    count=$(grep -cE "$pattern" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh || echo "0")
    if [ "$count" -gt 0 ]; then
...
if grep -A 10 "install_opencode_manager()" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | grep -q "command_exists npm"; then
...
if grep -A 10 "install_poetry()" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | grep -q "command_exists python3"; then
...
if grep -A 30 "opencode-manager.service" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | grep -q "ExecStart.*pnpm"; then
    test_pass "Systemd service includes pnpm ExecStart"
else
    test_fail "Systemd service missing pnpm ExecStart"
fi

if grep -A 30 "opencode-manager.service" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | grep -q "/usr/local/bin"; then
...
for pattern in "${ERROR_HANDLING_PATTERNS[@]}"; do
    if grep -q "$pattern" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh; then
...
NODEJS_LINE=$(grep -n "install_nodejs &&" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | head -1 | cut -d: -f1)
OPENCODE_LINE=$(grep -n "install_opencode_manager &&" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | head -1 | cut -d: -f1)
POETRY_LINE=$(grep -n "install_poetry &&" /workspace/repos/ubuntuserverinit/ubuntu-server-setup.sh | head -1 | cut -d: -f1)
...
if grep -q "Poetry" /workspace/repos/ubuntuserverinit/README.md; then
    test_pass "README.md documents Poetry"
else
    test_fail "README.md missing Poetry documentation"
fi

if grep -q "Poetry" /workspace/repos/ubuntuserverinit/README.zh-CN.md; then
